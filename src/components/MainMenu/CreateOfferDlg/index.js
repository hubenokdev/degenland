...
function CreateOfferDlg({
  myData,
  selectedFriendNftInfo,
  onClickSendOfferBtn,
  onClickCancelBtn
}) {
  ...
  const [selectedMyNftInfo, setSelectedMyNftInfo] = useState({});
  ...
  const [friendHbar, setFriendHbar] = useState(0);
  const [friendPal, setFriendPal] = useState(0);
  const [myHbar, setMyHbar] = useState(0);
  const [myPal, setMyPal] = useState(0);
  ...
  const getWalletBalance = async (accountId_) => {
    ...
    let g_palBalanceInfo = await getInfoResponse(`${env.MIRROR_NET_URL}/api/v1/tokens/${env.PAL_TOKEN_ID}/balances?account.id=${accountId_}`);
    console.log('getWalletBalance log - 2 : ', g_palBalanceInfo);
    if (!g_palBalanceInfo || g_palBalanceInfo.data.balances?.length === 0) {
      g_palBalance = 0;
    } else {
      g_palBalance = g_palBalanceInfo.data.balances[0].balance;
    }
  ...
  const getWalletNftData = async (nextLink_, accountId_) => {
    ...
    if (_result) {
      ...
      let placeInfo = await getInfoResponse(env.SERVER_URL + "/api/place/get_places_info?nftInfo=" + JSON.stringify(_tempData));
  ...
  const getFallbackFee = async (tokenId_, serialNum_) => {
    let g_fallback = 0;
  ...
  const handleAllRight = () => {
    ...
    for (let i = 0; i < selectedNormalNftInfo.length; i++) {
      if (selectedNormalNftInfo[i].tokenId == "plus") {
        const temp = selectedNormalNftInfo[selectedNormalNftInfo.length - 1];
        selectedNormalNftInfo[selectedNormalNftInfo.length - 1] = selectedNormalNftInfo[i];
        selectedNormalNftInfo[i] = temp;
      }
    }
  ...
  const handleCheckedRight = () => {
    ...
    //    setTickedCount(tickedCount - count);
    setSellNftCount(selectedMyNftInfo.length);
    setLandNftInfo(selectedLandNftInfo);
    setNormalNftInfo(selectedNormalNftInfo);
  };
        ...
        <Box style={{
          ...
          <Box sx={{
            ...
            <Box spacing={2}
              ...
              {
                selectedFriendNftInfo?.length > 0 &&
                selectedFriendNftInfo.map((item, index) => {
        ...
        <div
          ...
          <div style={{
            ...
            <Button
              ...
              onClick={async () => {
                //get fallback fee
                for (let i = 0;i < selectedFriendNftInfo.length;i++) {
                  if (selectedFriendNftInfo[i].nftType == 'normal')
                    selectedFriendNftInfo[i].fallback = await getFallbackFee(selectedFriendNftInfo[i].tokenId, selectedFriendNftInfo[i].serialNum);
                }
                for (let i = 0;i < selectedMyNftInfo.length;i++) {
                  if (selectedMyNftInfo[i].nftType == 'normal')
                    selectedMyNftInfo[i].fallback = await getFallbackFee(selectedMyNftInfo[i].tokenId, selectedMyNftInfo[i].serialNum);
                }
                ...
                if (parseInt(changeToRealValue(balanceInfo.pal, HBAR_DECIMAL), 10) < myPalBalance) {
                  toast.error("Insufficient PAL balance!");
                  return;
                }

                onClickSendOfferBtn(selectedFriendNftInfo, friendHbarBalance, friendPalBalance, selectedMyNftInfo, myHbarBalance, myPalBalance);
              }}
            >
              Send Offer
            </Button>
