...
function EditOfferDlg({
  ...
  const onClickApproveNfts = async (_offerId, _type, _tickedNfts, myHbar, myPal) => {
    ...
    for (let i = 0; i < _tickedNfts.length; i++)
      _fallbackFee += parseInt(_tickedNfts[i].fallbackFee, 10);
    ...
    for (let i = 0; i < _tickedNfts.length; i++) {
      ...
      _newStakedNftInfo.push({
        ...
        fallback: _tickedNfts[i].fallbackFee,
        ticked: false,
        staked: true,
        stakedDays: 0
      })
          ...
          <div style={{
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (result) {
                  await editOffer(offerInfo, offer.offerInfo.receiverNfts, friendHbar, friendPal, selectedMyNftInfo, myHbar, myPal);
                  const approveResult = await onClickApproveNfts(offerInfo._id, "provider", selectedMyNftInfo, myHbar, myPal);
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (friendHbar == '')
                  ...
                  friendHbarBalance = friendHbar;
                if (friendPal == '')
                  friendPalBalance = 0;
                else
                  friendPalBalance = friendPal;
