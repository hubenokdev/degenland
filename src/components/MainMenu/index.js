...
function MainMenu(props) {
    ...
    useEffect(() => {
        console.log("--------------------Complete loading notification log ---------------------");
    ...
    const onClickAlertMenuItem = async (alertItemInfo) => {
        if (alertItemInfo.alertType === ALERT_TYPE_INVITE_FRIEND) {
            if (alertItemInfo.state == 'unread') {
                ...
                await axios.post(env.SERVER_URL + "/api/notification/set_read", { id1: alertItemInfo.accountId, id2: alertItemInfo.playerInfo.accountId, state: alertItemInfo.state });
                props.props.loadNotification(playerinfoStore.data.accountId);
            }
        }
        else if (alertItemInfo.alertType === ALERT_TYPE_NFT_SWAP_OFFER) {
            // if (alertItemInfo.state == 'unread') {
            //     let res = await axios.get(env.SERVER_URL + "/api/nftswapoffer/get_offer?providerAccountId=" + alertItemInfo.playerInfo.accountId + "&offerId=" + alertItemInfo.alertId);

            //     setNftSwapOffer(res.data.data);
