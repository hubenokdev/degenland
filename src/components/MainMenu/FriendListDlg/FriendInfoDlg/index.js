...
function FriendInfoDlg({
    ...
    //    const [friendInfo, setFriendInfo] = useState(friend_test_info);
    const [friendInfo, setFriendInfo] = useState(selectedFriendInfo);
    ...
    useEffect(() => {
        if (friendInfo?.accountId && !uploadDataFlag) {
            getNftData(friendInfo.accountId);
            getWalletBalance(friendInfo.accountId);
        }
    }, [friendInfo]);
                                ...
                                return <Box key={index}
                                    ...
                                    <LandNftCard singleNftInfo={item} showDoneIcon={true}
                                        onClickLandNftCard={() => {
                                            ...
                                            if (item.ticked == true) {
                                                if (tickedCount < env.NFT_SELECT_LIMIT)
                                                    setTickedCount(tickedCount + 1);
                                                else {
                                                    item.ticked = item.ticked ? false : true;
                                                    toast.warning("You can get up to 7 NFTs with one offer.");
                                                }
                                            }
                                            else
                                                setTickedCount(tickedCount - 1);
                            ...
                            normalNftInfo.map((item, index) => {
                                return <Box key={index}
                                    ...
                                    <NormalNftCard singleNftInfo={item} showDoneIcon={true}
                                        onClickPlus={() => {
                                            getNextWalletNftData(nextLinkOfGetWalletNft, friendInfo.accountId);
                                        }}
